...
class FetchPersonImagesUseCaseImpl(
    private val apiService: TMDBService
) : FetchPersonImagesUseCase, ApiHandler {
    override fun execute(personId: String): Flow<ResponseStatus<Image>> {
        return flow {
            emit(handleApi { apiService.getPeopleImages(personId) })
        }.onStart {
